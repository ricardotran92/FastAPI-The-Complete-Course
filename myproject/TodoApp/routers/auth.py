...
router = APIRouter(
    prefix='/auth',
    tags=['auth']
) # add prefix 'auth' and group API endpoints

SECRET_KEY = 'b1f70e43ddf0a59a676c9869e717d2fc6d5609a3cfbf6209996fa682ff4a9a55' # $ openssl rand -hex 32
ALGORITHM = 'HS256'

bcrypt_context = CryptContext(schemes=['bcrypt'], deprecated='auto')
    # bcrypt is a popular and secure hashing algorithm for passwords
    # deprecated='auto' ensures that any deprecated hashing schemes are automatically handled
oauth2_bearer = OAuth2PasswordBearer(tokenUrl='auth/token') # verify the token as a dependency. 'auth/token' = prefix '/auth' + api endpoint '/token'
...
def create_access_token(username: str, user_id: int, role: str, expires_delta: timedelta):
    ...
    expires = datetime.now(timezone.utc) + expires_delta  # .utcnow() is deprecated → datetime.now(timezone.utc)
